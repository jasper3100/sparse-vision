...
def evaluate_feature_maps(original_activations_folder_path,
    ...
    print(f"KL divergence between original model's output and modified model's output: {kld:.4f}",
          f"Percentage of samples with the same classification (between original and modified model): {perc:.7f}%")
    if wandb_status:
        table_values = wandb.Table(columns=["KL-divergence", "Percentage same classification"], data=[[kld, perc]])
        wandb.log({"table_kld_percentage_same_classification": table_values}, commit=False)
    ...
    original_sparsity, mean_original_activated_units, mean_original_total_units = compute_sparsity(original_activated_units, original_total_units, train_dataset_length)
    ...
    adjusted_sparsity, mean_adjusted_activated_units, mean_adjusted_total_units = compute_sparsity(adjusted_activated_units, adjusted_total_units, train_dataset_length, expansion_factor=sae_params['expansion_factor'])
    # equivalently we could do: compute_sparsity(adjusted_activated_units, original_total_units) 
    # instead of providing the expansion factor, but sometimes we don't have access to original_total_units
    # hence, we use the function with the expansion factor
    ...
    # currently not in use, but logged directly during SAE training
    #'''
    ...
    mean_active_classes_per_neuron, std_active_classes_per_neuron = polysemanticity_level(encoder_output, target, num_classes, activation_threshold)
